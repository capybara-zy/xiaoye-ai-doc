...
Cursor 是基于 VS Code 的 AI 原生代码编辑器。在 Cursor 中使用 Claude Code，需要安装 Claude Code 官方插件，Cursor 原生的 Chat/Composer 功能无法直接替代 Claude Code。

## 前置条件

- 前往 [XiaoYe API控制台](https://ai.xiaoye.io) 获取 API Key
- 下载安装 [Cursor](https://cursor.com)
...
## Claude Code for Cursor 配置流程（共 4 步）
...
### 第三步：在 Cursor 里安装 Claude Code 插件

1. 打开 Cursor，按 `Ctrl+Shift+X` 打开扩展商店
2. 搜索 **Claude Code**
...
4. 保存文件（`Ctrl+S`），然后**重启 Cursor**
5. 再次点击侧边栏 **Claude Code** 图标，即可正常使用 ✅

## Cursor 原生 Chat / Composer（可选）

这是 Cursor 自带的 AI 功能，与 Claude Code 插件相互独立，按需配置。
...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 插件提示登录 / 无法连接 | settings.json 未配置或未重启 | 完成第四步配置后重启 Cursor |
